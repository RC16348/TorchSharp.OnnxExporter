...
{
    public class FractionalMaxPool2dProcessor : BaseProcessor<FractionalMaxPool2d>
    {
        public override string OpType => "FractionalMaxPool";

        public override DataFlowNode Process(FractionalMaxPool2d module, TraceContext context)
        {
            ...
            if (module.output_ratio is not null && module.output_ratio.Length >= 2)
            {
                node.Attributes["ratio"] = new[] { (float)module.output_ratio[0], (float)module.output_ratio[1] };
            }
            else
            {
                node.Attributes["ratio"] = new[] { 0.5f, 0.5f };
            }
